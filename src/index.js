...
class ExpressRoutePrinter {
  ...
  _processStack(stack, basePath, inherited) {
    const activeMiddleware = inherited.slice();
    for (const layer of stack) {
      if (layer.route) { this._processRouteLayer(layer, basePath, activeMiddleware); continue; }
      ...
      if (childStack) {
        ...
        if (mountPath === null) {
          ...
          this._processStack(childStack, basePath, activeMiddleware);
        } else this._processStack(childStack, this._joinPaths(basePath, mountPath), activeMiddleware);
        continue;
      }

      // A non-route, non-router layer is middleware registered with app.use()
      // or router.use(). It affects only layers registered after it.
      if (!layer.handle || layer.handle.length === 4 || this._isExpressInternal(layer)) continue;
      const mountPath = this._extractRouterPath(layer);
      activeMiddleware.push({
        name: handlerName(layer.handle),
        source: 'mounted',
        path: mountPath === null ? basePath || '/' : this._joinPaths(basePath, mountPath)
      });
    }
  }

  _isExpressInternal(layer) {
    return layer.name === 'query' || layer.name === 'expressInit';
  }

  _processRouteLayer(layer, basePath, inherited) {
    const route = layer.route;
    const path = this._joinPaths(basePath, route.path);
    const routeStack = route.stack || [];
    const directMiddleware = routeStack.filter((item, index) => index < routeStack.length - 1 || this._isMiddlewareName(handlerName(item.handle)))
      .map(item => ({ name: handlerName(item.handle), source: 'route', path }));
    const handlers = routeStack.filter((item, index) => index === routeStack.length - 1 && !this._isMiddlewareName(handlerName(item.handle)))
      .map(item => handlerName(item.handle));
    const mountedMiddleware = inherited.filter(item => this._middlewareApplies(item.path, path));
    const middlewareDetails = mountedMiddleware.concat(directMiddleware);
    const middleware = middlewareDetails.map(item => item.name);
    for (const method of Object.keys(route.methods || {})) {
      this.routes.push({ method: method.toUpperCase(), path, middlewareCount: middleware.length, middleware, middlewareDetails, handlerCount: handlers.length, handlers, layer });
    }
  }

  _isMiddlewareName(name) {
    return /auth|jwt|session|passport|guard|protect|verify|validate|rate.?limit|cors/i.test(name);
  }

  _middlewareApplies(middlewarePath, routePath) {
    return middlewarePath === '/' || routePath === middlewarePath || routePath.startsWith(`${middlewarePath}/`);
  }
  ...
  toJSON() { return this.routes.map(route => ({ method: route.method, path: route.path, middlewareCount: route.middlewareCount, middleware: route.middleware, middlewareDetails: route.middlewareDetails, handlerCount: route.handlerCount, handlers: route.handlers })); }
  ...
  getSecurityAudit() {
    ...
    }).map(route => ({ path: route.path, method: route.method, warning: 'No authentication middleware was identified; review this endpoint', middlewareCount: route.middlewareCount, middleware: route.middleware, handlers: route.handlers }));
  }
