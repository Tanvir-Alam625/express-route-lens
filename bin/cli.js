...
async function main() {
  ...
  const table = new Table({ head: ['Method', 'Path', 'Count', ...(opts.middleware ? ['Middleware'] : [])], colWidths: opts.middleware ? [10, 40, 8, 42] : [12, 68, 10] });
  for (const route of routes) table.push([route.method, route.path, String(route.middlewareCount), ...(opts.middleware ? [route.middleware.join(', ') || '-'] : [])]);
